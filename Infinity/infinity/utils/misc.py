...
class DistLogger(object):
    ...
    def __getattr__(self, attr: str):
        return (
            getattr(self._lg, attr) if self._lg is not None else DistLogger.do_nothing
        )
...
class MetricLogger(object):
    ...
    def log_every(
        self, start_it, max_iters, itrt, log_freq, log_every_iter=False, header=""
    ):  # also solve logging & skipping iterations before start_it
        ...
        if (
            ...
            for it in range(start_it, max_iters):
                ...
                if self.log_every_iter or it in self.log_iters:
                    ...
                    print(
                        f"{header_fmt.format(it, max_iters)}  eta: {str(datetime.timedelta(seconds=int(eta_seconds)))}  {str(self)}  T: {self.iter_time.value:.3f}s  dataT: {self.data_time.value*1e3:.1f}ms",
                        flush=True,
                    )
                self.iter_end_t = time.time()
        else:
        ...
        print(
            f"{header}   Cost of this ep:      {cost_str}   ({cost / (max_iters-start_it):.3f} s / it)",
            flush=True,
        )
...
def build_2d_sincos_position_embedding(
    h, w, embed_dim, temperature=10000.0, sc=0, verbose=True
):  # (1, hw**2, embed_dim)
    # DiT: sc=0
    # DETR: sc=2?
    ...
    assert (
        embed_dim % 4 == 0
    ), f"Embed dimension ({embed_dim}) must be divisible by 4 for 2D sin-cos position embedding!"
    pos_dim = embed_dim // 4
    omega = torch.arange(pos_dim, dtype=torch.float32) / pos_dim
    omega = (-math.log(temperature) * omega).exp()
    # omega == (1/T) ** (arange(pos_dim) / pos_dim), a vector only dependent on C
    out_w = grid_w * omega.view(
        1, pos_dim
    )  # out_w: scale * [0*ome, 0*ome, 0*ome, 1*ome, 1*ome, 1*ome, 2*ome, 2*ome, 2*ome]
    out_h = grid_h * omega.view(
        1, pos_dim
    )  # out_h: scale * [0*ome, 1*ome, 2*ome, 0*ome, 1*ome, 2*ome, 0*ome, 1*ome, 2*ome]
    pos_emb = torch.cat(
        [torch.sin(out_w), torch.cos(out_w), torch.sin(out_h), torch.cos(out_h)], dim=1
    )[None, :, :]
    if verbose:
        print(
            f"[build_2d_sincos_position_embedding @ {hw} x {hw}] scale_type={sc}, temperature={temperature:g}, shape={pos_emb.shape}"
        )
    ...
    for row, sc in enumerate(scs):
        for col, temp in enumerate(temps):
            ...
            pe = build_2d_sincos_position_embedding(
                hw, C, temperature=temp, sc=sc, verbose=False
            )[
                0
            ]  # N, C = 64, 16
            ...
            pe2 = build_2d_sincos_position_embedding(
                hw2, C, temperature=temp, sc=sc, verbose=False
            )[
                0
            ]  # N, C = 64, 16
            # pe2 = pe2.flip(dims=(0,))
            bchw, bchw2 = F.normalize(
                pe.view(hw, hw, C).permute(2, 0, 1).unsqueeze(0), dim=1
            ), F.normalize(pe2.view(hw2, hw2, C).permute(2, 0, 1).unsqueeze(0), dim=1)
            dis = [
            ...
            pe = pe.repeat_interleave(tar_w // pe.shape[0], dim=0).repeat_interleave(
                tar_h // pe.shape[1], dim=1
            )
