...
def precompute_rope2d_freqs_grid(
    ...
    t_height = torch.arange(max_height, device=device, dtype=torch.int64).type_as(
        inv_freq
    )
    t_width = torch.arange(max_width, device=device, dtype=torch.int64).type_as(
        inv_freq
    )
    ...
    for h_div_w in dynamic_resolution_h_w:
        ...
        for _, ph, pw in scale_schedule:
            ...
            if rope2d_normalized_by_hw == 1:  # downsample
                ...
                rope_cache = rope_cache.permute(
                    [0, 2, 3, 1]
                )  # (2, ph, pw, half_head_dim)
                ...
                raise ValueError(
                    f"Unknown rope2d_normalized_by_hw: {rope2d_normalized_by_hw}"
                )
...
class SelfAttention(nn.Module):
    ...
    def forward(
        self,
        x,
        attn_bias_or_two_vector: Union[
            torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]
        ],
        attn_fn=None,
        scale_schedule=None,
        ...
        if self.cos_attn:  # always True
            scale_mul = self.scale_mul_1H11.clamp_max(
                self.max_scale_mul
            ).exp()  # 11H1 (flash), or 1H11 (not flash)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv):
        ...
        kv_compact = F.linear(
            ...
            N, 2, self.num_heads, self.head_dim
        )  # NC => N2Hc
...
class AdaLNBeforeHead(nn.Module):
    def __init__(
        self, C, D, act: bool, norm_layer: partial, fused_norm_func=None
